...
const DataManagementView: React.FC = () => {
  ...
  const handleSaveFfaConfig = async () => {
    ...
    try {
      ...
      const intervalMins = Math.max(3, scheduleIntervalMinutes);
      if (scheduleEnabled && scheduleMode === 'interval' && scheduleIntervalMinutes < 3) {
        showToast('Interval must be at least 3 minutes', 'error');
        return;
      }
                  ...
                  {ffaDataSource === 'api' && scheduleEnabled && (
                    <div className="space-y-3 pl-7">
                      ...
                      {scheduleMode === 'interval' && (
                        <div>
                          <label className="block text-xs font-bold text-slate-600 mb-1">Interval (minutes, min 3)</label>
                          <input
                            type="number"
                            min={3}
                            max={10080}
